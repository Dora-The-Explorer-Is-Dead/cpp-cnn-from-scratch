...
Convolutional_Layer::Convolutional_Layer(int r, int c, int k, int f, int s, string a, const vector<vector<vector<vector<double>>>>& i) : rows_no(r), cols_no(c), kernels_no(k), filters_no(f), stride(s), activation_function(a), images(i) {
    ...
    filters.resize(filters_no);
    for (int i = 0; i < filters_no; i++) {
        filters[i].resize(kernels_no);
        for (int j = 0; j < kernels_no; j++) {
            filters[i][j].resize(rows_no, vector<double>(cols_no));
            for (int k = 0; k < rows_no; k++) {
                for (int l = 0; l < cols_no; l++) {
                    filters[i][j][k][l] = 0.01 * ((2.0 * (rand() / (RAND_MAX + 1.0))) - 1.0);
                }
            }
        }
    }
